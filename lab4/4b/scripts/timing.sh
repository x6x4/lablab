...
do
    sum_s=0
    sed -n "$(($EPOCHS*i-$EPOCHS + 1)), $(($EPOCHS*i))p" ord_search | awk '{sum_s += $1} END {x=sum_s/'$EPOCHS'*10; print x}'
done
...
do
    sum_i=0
    sed -n "$(($EPOCHS*i-$EPOCHS + 1)), $(($EPOCHS*i))p" ord_ins | awk '{sum_i += $1} END {x=sum_i/'$EPOCHS'*10; print x}'
done
...
do
    sum_d=0
    sed -n "$(($EPOCHS*i-$EPOCHS + 1)), $(($EPOCHS*i))p" ord_del | awk '{sum_d += $1} END {x=sum_d/'$EPOCHS'*10; print x}'
done
...
do
    sum_t=0
    sed -n "$(($EPOCHS*i-$EPOCHS + 1)), $(($EPOCHS*i))p" ord_trav | awk '{sum_t += $1} END {x=sum_t/'$EPOCHS'*10; print x}'
done
